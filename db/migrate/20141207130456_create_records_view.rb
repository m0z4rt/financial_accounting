class CreateRecordsView < ActiveRecord::Migration
  def change
    execute <<-SQL
    CREATE OR REPLACE VIEW incomes_expenses_view AS
    SELECT incomes.id as record_id, incomes.name, incomes.income AS record_value, incomes.account_id,
    ...
    UNION
    SELECT expenses.id  as record_id, expenses.name, expenses.expense AS record_value, expenses.account_id,
